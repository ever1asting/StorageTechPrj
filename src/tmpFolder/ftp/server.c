...
Boolean service_handleCmd(const int a_socket, const String *ap_argv, const int a_argc)
{
	...
	// my code
	else if(strcmp(ap_argv[0], "sizels") == 0)
	{
		// service_readDir(g_pwd, &dataBufLen)
		// pass args
		const String a_path = malloc(sizeof(char*) * PATH_MAX);
		strcpy(a_path, g_pwd);
		int* ap_length = &dataBufLen;
		...
		else
		{
			...
			{
				struct stat e;
				lstat(p_dirInfo->d_name, &e);
				char str[PATH_MAX + 100];
				sprintf(str, "%d", e.st_size);
				strcat(str, " ");
				strcat(str, p_dirInfo->d_name);
